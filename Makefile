...
# Build the project
all: link clean test vet linux darwin windows freebsd
...
freebsd:
	cd ${BUILD_DIR}; \
	GOOS=freebsd GOARCH=${GOARCH} go build -o ${BINARY}-freebsd-${GOARCH} . ; \
	cd - >/dev/null
...
.PHONY: link linux darwin windows freebsd test vet fmt clean
